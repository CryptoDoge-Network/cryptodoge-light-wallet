...
export default function Block() {
  ...
  const poolReward = mojo_to_cryptodogelight(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_cryptodogelight(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const cryptodogelightFees = blockRecord.fees
    ? mojo_to_cryptodogelight(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/cryptodoge/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/cryptodoge/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: cryptodogelightFees ? `${cryptodogelightFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Cryptodoge blockchain
            </Trans>
          </Back>
        }
        action={

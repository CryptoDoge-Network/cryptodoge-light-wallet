...
import { mojo_to_cryptodogelight, cryptodogelight_to_mojo } from '../../util/cryptodogelight';
...
export default function PlotNFTAbsorbRewards(props: Props) {
  ...
  async function handleSubmit(data: SubmitData) {
    try {
      ...
      const feeMojos = cryptodogelight_to_mojo(fee);
          ...
          >
            <Typography variant="subtitle1">
              <Trans>
                You will recieve{' '}
                <UnitFormat
                  value={mojo_to_cryptodogelight(BigInt(balance))}
                  display="inline"
                  state={State.SUCCESS}
                />{' '}
                to {wallet?.address}

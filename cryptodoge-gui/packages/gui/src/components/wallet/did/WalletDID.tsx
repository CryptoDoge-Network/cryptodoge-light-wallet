...
import { AlertDialog, Card, Flex, Loading, Dropzone } from '@cryptodogelight/core';
import {
...
import { mojo_to_cryptodogelight_string } from '../../../util/cryptodogelight';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_cryptodogelight_string(props.balance)} TXCD
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const CreateAttest = (props) => {
  ...
  function createAttestPacket() {
    ...
    if (address.substring(0, 12) === 'cryptodogelight_addr://') {
...
const CashoutCard = (props) => {
  ...
  function cashout() {
    ...
    if (puzzlehash.slice(0, 12) === 'cryptodogelight_addr://') {

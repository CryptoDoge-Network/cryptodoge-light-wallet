...
import { AlertDialog, Card, Flex } from '@cryptodogelight/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const cryptodogelightper_input = ip_parsed.limit;
    ...
    const cryptodogelightper_value = Number.parseInt(Number(cryptodogelightper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (cryptodogelight per interval):{' '}
                  {mojo_to_cryptodogelight_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = cryptodogelight_to_mojo(fee_input.value);

...
import { AlertDialog } from '@cryptodogelight/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  let cryptodogelightper_input = null;
  ...
  function create() {
    ...
    if (
      cryptodogelightper_input.value === '' ||
      Number(cryptodogelightper_input.value) === 0 ||
      !Number(cryptodogelightper_input.value) ||
      isNaN(Number(cryptodogelightper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const cryptodogelightper = cryptodogelight_to_mojo(cryptodogelightper_input.value);
    const cryptodogelightper_value = Number.parseInt(Number(cryptodogelightper));
    const userpubkey = userpubkey_input.value;
    const amount = cryptodogelight_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = cryptodogelight_to_mojo(fee_input.value);
    // TODO(lipa): send fee to server
    // const fee_value = parseInt(Number(fee));
    dispatch(
      create_rl_admin_action(
        interval_value,
        cryptodogelightper_value,
        userpubkey,
        amount_value,
      ),
    );
        ...
        <Box display="flex">
          ...
          <Box flexGrow={6}>
            <TextField
              ...
              fullWidth
              inputRef={(input) => {
                cryptodogelightper_input = input;
              }}

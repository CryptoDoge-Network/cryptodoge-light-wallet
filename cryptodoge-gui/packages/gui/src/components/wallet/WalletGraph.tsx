...
// import { Flex, FormatLargeNumber } from '@cryptodogelight/core';
...
function prepareGraphPoints(
  ...
  */
  ...
  const points = [
    {
      x: peak.height,
      y: Math.max(0, mojo_to_cryptodogelight(start)),
      tooltip: mojo_to_cryptodogelight(balance),
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: Math.max(0, mojo_to_cryptodogelight(start)),
      tooltip: mojo_to_cryptodogelight(start),
    });
  });

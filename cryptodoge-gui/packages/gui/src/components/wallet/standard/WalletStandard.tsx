...
import {
  ...
  Fee,
  Form,
  TextField as CryptodogeTextField,
  AlertDialog,
  CopyToClipboard,
  Flex,
  Card,
  ConfirmDialog,
} from '@cryptodogelight/core';
...
import { /* mojo_to_cryptodogelight_string, */ cryptodogelight_to_mojo } from '../../../util/cryptodogelight';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_cryptodogelight_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of cryptodogelight in the blockchain at the current
      ...
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the amount of Cryptodoge that you can currently use to make
            transactions. It does not include pending farming rewards, pending
            incoming transactions, and Cryptodoge that you have just spent but is not
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send cryptodogelight to coloured address. Please enter a cryptodogelight
              address.
            </Trans>
          </AlertDialog>,
        ),
    ...
    if (address.slice(0, 12) === 'cryptodogelight_addr://') {
    ...
    const amountValue = Number.parseFloat(cryptodogelight_to_mojo(amount));
    const feeValue = Number.parseFloat(cryptodogelight_to_mojo(fee));

...
// global.daemon_rpc_ws = `wss://${self_hostname}:57692`;
...
async function loadConfig(net) {
  try {
    // check if CRYPTODOGE_ROOT is set. it overrides 'net'
    const config_root_dir =
      'CRYPTODOGE_ROOT' in process.env
        ? process.env.CRYPTODOGE_ROOT
        : path.join(os.homedir(), '.cryptodogelight', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 57692); // jshint ignore:line

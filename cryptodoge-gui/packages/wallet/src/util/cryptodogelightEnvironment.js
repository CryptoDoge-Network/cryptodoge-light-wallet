...
const PY_FOLDER = '../../../cryptodogelight/daemon';
...
const getCryptodogeVersion = () => {
  let version = null;
  const exePath = getExecutablePath('cryptodogelight');
  // first see if we can get a cryptodogelight exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or cryptodogelight is on the path
    try {
      version = child_process
...
module.exports = {
  startCryptodogeDaemon,
  getCryptodogeVersion,
  guessPackaged,
};

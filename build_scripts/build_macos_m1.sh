...
electron-packager . "Cryptodoge Light Wallet" --asar.unpack="**/daemon/**" --platform=darwin \
...
if [ "$NOTARIZE" ]; then
  electron-osx-sign Cryptodoge\ Light\ Wallet-darwin-arm64/Cryptodoge\ Light\ Wallet.app --platform=darwin \
  --hardened-runtime=true --provisioning-profile=cryptodogelightblockchain.provisionprofile \
...
mv Cryptodoge\ Light\ Wallet-darwin-arm64 ../../../build_scripts/dist/
cd ../../../build_scripts || exit

DMG_NAME="CryptodogeLight-$CRYPTODOGE_INSTALLER_VERSION-arm64.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/Cryptodoge\ Light\ Wallet-darwin-arm64/Cryptodoge\ Light\ Wallet.app Cryptodoge-$CRYPTODOGE_INSTALLER_VERSION-arm64 \
...
if [ "$NOTARIZE" ]; then
  ...
  notarize-cli --file=$DMG_NAME --bundle-id org.cryptodogenetwork.lightwallet \
...
# xcrun altool --notarize-app -f Cryptodoge-0.1.X.dmg --primary-bundle-id org.cryptodogenetwork.lightwallet -u username -p password

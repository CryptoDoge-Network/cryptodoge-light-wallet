...
from cryptodogelight.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from cryptodogelight.full_node.full_node_api import FullNodeAPI
from cryptodogelight.server.start_farmer import service_kwargs_for_farmer
from cryptodogelight.server.start_full_node import service_kwargs_for_full_node
from cryptodogelight.server.start_harvester import service_kwargs_for_harvester
from cryptodogelight.server.start_introducer import service_kwargs_for_introducer
from cryptodogelight.server.start_service import Service
from cryptodogelight.server.start_timelord import service_kwargs_for_timelord
from cryptodogelight.server.start_wallet import service_kwargs_for_wallet
from cryptodogelight.simulator.start_simulator import service_kwargs_for_full_node_simulator
from cryptodogelight.timelord.timelord_launcher import kill_processes, spawn_process
...
from cryptodogelight.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["xcd_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xcd")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xcd_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xcd")

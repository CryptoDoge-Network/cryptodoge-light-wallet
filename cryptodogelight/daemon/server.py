...
from cryptodogelight.cmds.init_funcs import cryptodogelight_init
from cryptodogelight.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from cryptodogelight.daemon.keychain_server import KeychainServer, keychain_commands
from cryptodogelight.daemon.windows_signal import kill
from cryptodogelight.server.server import ssl_context_for_root, ssl_context_for_server
from cryptodogelight.ssl.create_ssl import get_mozilla_ca_crt
from cryptodogelight.util.cryptodogelight_logging import initialize_logging
from cryptodogelight.util.config import load_config
from cryptodogelight.util.json_util import dict_to_json_str
from cryptodogelight.util.keychain import (
    Keychain,
...
from cryptodogelight.util.service_groups import validate_service
from cryptodogelight.util.setproctitle import setproctitle
from cryptodogelight.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Cryptodoge.")
    quit()
...
service_plotter = "cryptodogelight plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "cryptodogelight": "cryptodogelight",
        "cryptodogelight_wallet": "start_wallet",
        "cryptodogelight_full_node": "start_full_node",
        "cryptodogelight_harvester": "start_harvester",
        "cryptodogelight_farmer": "start_farmer",
        "cryptodogelight_introducer": "start_introducer",
        "cryptodogelight_timelord": "start_timelord",
        "cryptodogelight_timelord_launcher": "timelord_launcher",
        "cryptodogelight_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("cryptodogelight daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered CRYPTODOGE_ROOT
    os.environ["CRYPTODOGE_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with CRYPTODOGE_ROOT: {os.environ['CRYPTODOGE_ROOT']}")
    ...
    if service_command == "cryptodogelight_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in cryptodogelight_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    cryptodogelight_init(root_path, should_check_keys=False)
    config = load_config(root_path, "config.yaml")
    setproctitle("cryptodogelight_daemon")

...
DEFAULT_USER = f"user-cryptodoge-{CURRENT_KEY_VERSION}"  # e.g. user-cryptodogelight-1.8
DEFAULT_SERVICE = f"chia-{DEFAULT_USER}"  # e.g. chia-user-cryptodogelight-1.8
...
def supports_keyring_passphrase() -> bool:
    return False or os.environ.get("CRYPTODOGE_PASSPHRASE_SUPPORT", "").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from cryptodogelight.cmds.passphrase_funcs import prompt_for_passphrase
...
class Keychain:
    ...
    def migrate_legacy_keyring(
        ...
        """
        raise Exception('Keyring migration is not supported by the Cryptodoge Light Wallet.')

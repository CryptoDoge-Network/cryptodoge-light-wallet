...
from cryptodogelight.protocols import full_node_protocol, introducer_protocol
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: CryptodogeServer = server
    ...
    async def on_connect(self, peer: ws.WSCryptodogeConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSCryptodogeConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSCryptodogeConnection):
    ...
    async def on_connect_callback(self, peer: ws.WSCryptodogeConnection):

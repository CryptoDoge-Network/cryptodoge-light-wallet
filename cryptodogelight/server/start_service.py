...
from cryptodogelight.daemon.server import singleton, service_launch_lock_path
from cryptodogelight.server.ssl_context import cryptodogelight_ssl_ca_paths, private_ssl_ca_paths
...
from cryptodogelight.server.upnp import UPnP
...
from cryptodogelight.util.config import load_config, load_config_cli
...
class Service:
    def __init__(
        ...
        proctitle_name = f"cryptodogelight_{service_name}"
        ...
        cryptodogelight_ca_crt, cryptodogelight_ca_key = cryptodogelight_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = CryptodogeServer(
            advertised_port,
            node,
            peer_api,
            node_type,
            ...
            (cryptodogelight_ca_crt, cryptodogelight_ca_key),
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for CryptodogeServer to be closed")

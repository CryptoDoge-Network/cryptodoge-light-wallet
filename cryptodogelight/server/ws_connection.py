...
from cryptodogelight.cmds.init_funcs import cryptodogelight_full_version_str
from cryptodogelight.protocols.protocol_message_types import ProtocolMessageTypes
from cryptodogelight.protocols.protocol_state_machine import message_response_ok
from cryptodogelight.protocols.protocol_timing import INTERNAL_PROTOCOL_ERROR_BAN_SECONDS
from cryptodogelight.protocols.shared_protocol import Capability, Handshake
...
from cryptodogelight.util.ints import uint8, uint16

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from cryptodogelight.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSCryptodogeConnection:
    """
    ...
    async def perform_handshake(self, network_id: str, protocol_version: str, server_port: int, local_type: NodeType):
        if self.is_outbound:
            outbound_handshake = make_msg(
                ...
                Handshake(
                    'cryptodoge-' + network_id,
                    protocol_version,
                    cryptodogelight_full_version_str(),
                    uint16(server_port),
            ...
            if inbound_handshake.network_id != 'cryptodoge-' + network_id:

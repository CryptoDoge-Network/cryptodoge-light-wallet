...
from cryptodogelight.consensus.find_fork_point import find_fork_point_in_chain
from cryptodogelight.daemon.keychain_proxy import (
...
from cryptodogelight.full_node.weight_proof import chunks
from cryptodogelight.protocols import wallet_protocol
from cryptodogelight.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from cryptodogelight.server.node_discovery import WalletPeers
...
from cryptodogelight.types.weight_proof import WeightProof, SubEpochData
from cryptodogelight.util.byte_types import hexstr_to_bytes
from cryptodogelight.util.ints import uint32, uint64
from cryptodogelight.util.keychain import KeyringIsLocked
from cryptodogelight.util.path import mkdir, path_from_root
from cryptodogelight.wallet.block_record import HeaderBlockRecord
...
from cryptodogelight.wallet.util.wallet_sync_utils import (
    validate_additions,
    validate_removals,
...
from cryptodogelight.wallet.wallet_action import WalletAction
...
class WalletNode:
    ...
    server: Optional[CryptodogeServer]
    log: logging.Logger
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'cryptodogelight keys' program.")
            return None
    ...
    async def new_puzzle_hash_created(self, puzzle_hashes: List[bytes32]):
        if len(puzzle_hashes) == 0:
            return
        assert self.server is not None
        full_nodes: Dict[bytes32, WSCryptodogeConnection] = self.server.connection_by_type.get(NodeType.FULL_NODE, {})
    ...
    async def on_connect(self, peer: WSCryptodogeConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def trusted_sync(self, full_node: WSCryptodogeConnection):
        """
    ...
    async def subscribe_to_phs(self, puzzle_hashes: List[bytes32], peer: WSCryptodogeConnection, height=uint32(0)):
        """
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSCryptodogeConnection):
        assert self.wallet_state_manager is not None
        assert self.server is not None
    ...
    async def fetch_last_tx_from_peer(self, height: uint32, peer: WSCryptodogeConnection) -> Optional[HeaderBlock]:
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSCryptodogeConnection):
        assert self.wallet_state_manager is not None
        assert self.server is not None
        async with self.new_peak_lock:
            if self.wallet_state_manager is None:
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSCryptodogeConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def get_additions(
        self, peer: WSCryptodogeConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSCryptodogeConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSCryptodogeConnection, peak: HeaderBlock
    ...
    async def untrusted_sync_to_peer(
        self, peer: WSCryptodogeConnection, weight_proof: WeightProof, syncing: bool, fork_height: int
    ):
        assert self.wallet_state_manager is not None

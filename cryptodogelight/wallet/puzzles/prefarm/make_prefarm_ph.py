...
address2 = "txcd1c2cguswhvmdyz9hr3q6hak2h6p9dw4rz82g4707k2xy2sarv705qcce4pn"  # Mariano address (m/12381/8444/2/0)
...
def make_puzzle(amount: int) -> int:
    ...
    if error:
        ...
        for cvp in result_human:
            ...
            total_cryptodogelight += int_from_bytes(cvp.vars[1])
            print(
                f"{ConditionOpcode(cvp.opcode).name}: {encode_puzzle_hash(cvp.vars[0], prefix)},"
                f" amount: {int_from_bytes(cvp.vars[1])}"
            )
    return total_cryptodogelight


total_cryptodogelight = 0
print("Pool address: ")
total_cryptodogelight += make_puzzle(pool_amounts)
print("\nFarmer address: ")
total_cryptodogelight += make_puzzle(farmer_amounts)

assert total_cryptodogelight == calculate_base_farmer_reward(uint32(0)) + calculate_pool_reward(uint32(0))

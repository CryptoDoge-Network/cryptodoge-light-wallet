...
from cryptodogelight.protocols.wallet_protocol import PuzzleSolutionResponse, CoinState
...
from cryptodogelight.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict
from cryptodogelight.util.ints import uint8, uint32, uint64, uint128
from cryptodogelight.util.json_util import dict_to_json_str
from cryptodogelight.wallet.cc_wallet.cat_constants import DEFAULT_CATS
from cryptodogelight.wallet.cc_wallet.cc_info import CCInfo
...
from cryptodogelight.wallet.derivation_record import DerivationRecord
from cryptodogelight.wallet.lineage_proof import LineageProof
from cryptodogelight.wallet.puzzles.genesis_checkers import ALL_LIMITATIONS_PROGRAMS
...
from cryptodogelight.wallet.util.transaction_type import TransactionType
from cryptodogelight.wallet.util.wallet_types import WalletType
from cryptodogelight.wallet.wallet import Wallet
from cryptodogelight.wallet.wallet_coin_record import WalletCoinRecord
from cryptodogelight.wallet.wallet_info import WalletInfo
...
class CCWallet:
    ...
    async def create_new_cc_wallet(
        ...
        try:
            cryptodogelight_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        cryptodogelight_tx = replace(cryptodogelight_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(cryptodogelight_tx)
    ...
    async def create_tandem_xcd_tx(
        self,
        fee: uint64,
        ...
        if fee > amount_to_claim:
            cryptodogelight_coins = await self.standard_wallet.select_coins(fee)
            origin_id = list(cryptodogelight_coins)[0].name()
            selected_amount = sum([c.amount for c in cryptodogelight_coins])
            cryptodogelight_tx = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
                (await self.standard_wallet.get_new_puzzlehash()),
                fee=uint64(fee - amount_to_claim),
                coins=cryptodogelight_coins,
            ...
            assert cryptodogelight_tx.spend_bundle is not None

            message = None
            for spend in cryptodogelight_tx.spend_bundle.coin_spends:
        ...
        return cryptodogelight_tx, announcement
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some XCD as part of this
        regular_cryptodogelight_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_cryptodogelight_to_claim = payment_amount

        need_cryptodogelight_transaction = (fee > 0 or regular_cryptodogelight_to_claim > 0) and (fee - regular_cryptodogelight_to_claim != 0)
        ...
        spendable_cc_list = []
        cryptodogelight_tx = None
        first = True
        for coin in cat_coins:
            if first:
                first = False
                if need_cryptodogelight_transaction:
                    if fee > regular_cryptodogelight_to_claim:
                        announcement = Announcement(coin.name(), b"$", b"\xca")
                        cryptodogelight_tx, _ = await self.create_tandem_xcd_tx(
                            fee, uint64(regular_cryptodogelight_to_claim), announcement_to_assert=announcement
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries, coin_announcements={announcement.message}
                        )
                    elif regular_cryptodogelight_to_claim > fee:
                        cryptodogelight_tx, _ = await self.create_tandem_xcd_tx(fee, uint64(regular_cryptodogelight_to_claim))
                        innersol = self.standard_wallet.make_solution(
        ...
        cryptodogelight_spend_bundle = SpendBundle([], G2Element())
        if cryptodogelight_tx is not None and cryptodogelight_tx.spend_bundle is not None:
            cryptodogelight_spend_bundle = cryptodogelight_tx.spend_bundle

        return (
            SpendBundle.aggregate(
                [
                    cat_spend_bundle,
                    cryptodogelight_spend_bundle,
                ]
            ),
            cryptodogelight_tx,
        )
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, cryptodogelight_tx = await self.generate_unsigned_spendbundle(payments, fee, coins=coins)
        ...
        if cryptodogelight_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=cryptodogelight_tx.confirmed_at_height,
                    created_at_time=cryptodogelight_tx.created_at_time,
                    to_puzzle_hash=cryptodogelight_tx.to_puzzle_hash,
                    amount=cryptodogelight_tx.amount,
                    fee_amount=cryptodogelight_tx.fee_amount,
                    confirmed=cryptodogelight_tx.confirmed,
                    sent=cryptodogelight_tx.sent,
                    spend_bundle=None,
                    additions=cryptodogelight_tx.additions,
                    removals=cryptodogelight_tx.removals,
                    wallet_id=cryptodogelight_tx.wallet_id,
                    sent_to=cryptodogelight_tx.sent_to,
                    trade_id=cryptodogelight_tx.trade_id,
                    type=cryptodogelight_tx.type,
                    name=cryptodogelight_tx.name,
                    memos=[],
                )
            )

...
from cryptodogelight.wallet.puzzles.puzzle_utils import (
...
from cryptodogelight.wallet.secret_key_store import SecretKeyStore
from cryptodogelight.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_cryptodogelight(self, cryptodogelight_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if cryptodogelight_amount < 0:
            utxos = await self.select_coins(abs(cryptodogelight_amount), exclude)
        else:
        ...
        cryptodogelight_amount = spend_value + cryptodogelight_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": cryptodogelight_amount}]

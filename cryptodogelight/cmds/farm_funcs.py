...
from cryptodogelight.cmds.units import units
...
from cryptodogelight.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total cryptodogelight farmed: {amounts['farmed_amount'] / units['cryptodogelight']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['cryptodogelight']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['cryptodogelight']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'cryptodogelight start wallet' and 'cryptodogelight wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'cryptodogelight wallet show'")
    else:
        print("Note: log into your key using 'cryptodogelight wallet show' to see rewards for each key")

...
from cryptodogelight import __version__
from cryptodogelight.consensus.coinbase import create_puzzlehash_for_pk
from cryptodogelight.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_cryptodogelight_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from cryptodogelight.util.bech32m import encode_puzzle_hash
from cryptodogelight.util.config import (
    create_default_cryptodogelight_config,
    initial_config_file,
    load_config,
    save_config,
    unflatten_properties,
)
from cryptodogelight.util.keychain import Keychain
from cryptodogelight.util.path import mkdir
from cryptodogelight.util.ssl_check import (
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'cryptodogelight keys generate'")
        return None
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-2] == config["farmer"].get("xcd_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-2] == config["pool"].get("xcd_target_address"):
                stop_searching_for_pool = True
    ...
    if "xcd_target_address" not in config["farmer"]:
        print(
            f"Setting the xcd destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
    ...
    if "xcd_target_address" not in config["pool"]:
        print(f"Setting the xcd destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
    ...
    if updated_target:
        print(
            f"To change the XCD destination addresses, edit the `xcd_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    cryptodogelight_ca_crt, cryptodogelight_ca_key = get_cryptodogelight_ca_crt_key()
    cryptodogelight_ca_crt_path = ca_dir / "cryptodogelight_ca.crt"
    cryptodogelight_ca_key_path = ca_dir / "cryptodogelight_ca.key"
    write_ssl_cert_and_key(cryptodogelight_ca_crt_path, cryptodogelight_ca_crt, cryptodogelight_ca_key_path, cryptodogelight_ca_key)
    ...
    cryptodogelight_ca_crt, cryptodogelight_ca_key = get_cryptodogelight_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, cryptodogelight_ca_crt, cryptodogelight_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if cryptodogelight_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return cryptodogelight_init(root_path, fix_ssl_permissions=fix_ssl_permissions)


def cryptodogelight_version_number() -> Tuple[str, str, str, str]:
...
def cryptodogelight_minor_release_number():
    res = int(cryptodogelight_version_number()[2])
    print(f"Install release number: {res}")
    return res


def cryptodogelight_full_version_str() -> str:
    major, minor, patch, dev = cryptodogelight_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def cryptodogelight_init(root_path: Path, *, should_check_keys: bool = False, fix_ssl_permissions: bool = False):
    """
    ...
    if os.environ.get("CRYPTODOGE_ROOT", None) is not None:
        print(
            ...
            f"Please unset the environment variable and run cryptodogelight init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Cryptodoge directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if CRYPTODOGE_ROOT is set, or if user has run cryptodogelight init twice
        # before a new update.
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'cryptodogelight keys show --show-mnemonic-seed'")

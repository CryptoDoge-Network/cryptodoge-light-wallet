...
def init_cmd(ctx: click.Context, create_certs: str, fix_ssl_permissions: bool, **kwargs):
    ...
    - Make a copy of your Farming Machine CA directory: ~/.cryptodogelight/[version]/config/ssl/ca
    - Shut down all cryptodogelight daemon processes with `cryptodogelight stop all -d`
    - Run `cryptodogelight init -c [directory]` on your remote harvester,
    ...
    from cryptodogelight.cmds.passphrase_funcs import initialize_passphrase
...
if __name__ == "__main__":
    from .init_funcs import cryptodogelight_init
    from cryptodogelight.util.default_root import DEFAULT_ROOT_PATH

    cryptodogelight_init(DEFAULT_ROOT_PATH)

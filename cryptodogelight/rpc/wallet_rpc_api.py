...
from cryptodogelight.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
from cryptodogelight.protocols.protocol_message_types import ProtocolMessageTypes
from cryptodogelight.server.outbound_message import NodeType, make_msg
...
from cryptodogelight.util.keychain import KeyringIsLocked, bytes_to_mnemonic, generate_mnemonic
from cryptodogelight.util.path import path_from_root
from cryptodogelight.util.ws_message import WsRpcMessage, create_payload_dict
...
from cryptodogelight.wallet.derive_keys import master_sk_to_singleton_owner_sk, master_sk_to_wallet_sk_unhardened
from cryptodogelight.wallet.rl_wallet.rl_wallet import RLWallet
from cryptodogelight.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
...
from cryptodogelight.wallet.util.trade_utils import trade_record_to_dict
...
from cryptodogelight.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "cryptodogelight_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "cryptodogelight_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xcd_target_address")
        pool_target = config["pool"].get("xcd_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                from cryptodogelight.pools.pool_wallet_info import initial_pool_state_from_dict
